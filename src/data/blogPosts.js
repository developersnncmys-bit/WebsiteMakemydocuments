...
// ─────────────────────────────────────────────────────────────
// API integration — blogs are managed in the admin panel and stored in the
// backend. The backend stores plain fields (title, category, excerpt, content…);
// we map those to the presentational shape the website renders, deriving the
...
const CAT_PRESENTATION = {
  'Passport':                     { Icon: BookOpen,       gradient: 'linear-gradient(145deg, #0f2d5c 0%, #2E68B1 60%, #3d8bd4 100%)' },
  'Tourist Visa':                 { Icon: Plane,          gradient: 'linear-gradient(145deg, #7c1d05 0%, #c2410c 55%, #f97316 100%)' },
  'PAN Card':                     { Icon: CreditCard,     gradient: 'linear-gradient(145deg, #3b0764 0%, #7c3aed 55%, #a78bfa 100%)' },
  'Senior Citizen Card':          { Icon: Users,          gradient: 'linear-gradient(145deg, #064e3b 0%, #059669 55%, #34d399 100%)' },
  'Insurance':                    { Icon: ShieldCheck,    gradient: 'linear-gradient(145deg, #0c4a6e 0%, #0369a1 55%, #0ea5e9 100%)' },
  'Rental Agreement':             { Icon: Home,           gradient: 'linear-gradient(145deg, #431407 0%, #9a3412 55%, #ea580c 100%)' },
  'Lease Agreement':              { Icon: FileText,       gradient: 'linear-gradient(145deg, #4a044e 0%, #86198f 55%, #e879f9 100%)' },
  'Police Verification':          { Icon: BadgeCheck,     gradient: 'linear-gradient(145deg, #052e16 0%, #166534 55%, #22c55e 100%)' },
  'MSME Certificate':             { Icon: Building2,      gradient: 'linear-gradient(145deg, #1a1a4e 0%, #3730a3 55%, #6366f1 100%)' },
  'Police Clearance Certificate': { Icon: ClipboardCheck, gradient: 'linear-gradient(145deg, #0f172a 0%, #1e3a5f 55%, #2563eb 100%)' },
  'Affidavits / Annexure':        { Icon: Globe,          gradient: 'linear-gradient(145deg, #334155 0%, #475569 55%, #94a3b8 100%)' },
}
...
function mapApiBlog(b) {
  const pres = CAT_PRESENTATION[b.category] || DEFAULT_PRES
  return {
    id: b.id || b._id,
    slug: b.slug,
    cat: b.category || 'Passport',
    featured: false,
    title: b.title || '',
    excerpt: b.excerpt || '',
...
export async function fetchBlogPosts() {
  try {
    ...
    const apiPosts = json && json.success && Array.isArray(json.data) ? json.data.map(mapApiBlog) : []
    const seen = new Set(apiPosts.map(p => p.slug))
    return [...apiPosts, ...BLOG_POSTS.filter(p => !seen.has(p.slug))]
  } catch {
    return BLOG_POSTS
  }
}
...
export async function fetchBlogBySlug(slug) {
  try {
    ...
    if (res.ok) {
      const json = await res.json()
      if (json && json.success && json.data) return mapApiBlog(json.data)
    }
  } catch {}
  return BLOG_POSTS.find(p => p.slug === slug) || null
}

// local-only lookup, kept for any synchronous callers
export function getBlogBySlug(slug) {
  return BLOG_POSTS.find(p => p.slug === slug) || null
}

export function getRelatedPosts(posts, currentId, cat, count = 3) {
  const list = Array.isArray(posts) ? posts : BLOG_POSTS
  return list
    .filter(p => p.id !== currentId && p.cat === cat)
    .concat(list.filter(p => p.id !== currentId && p.cat !== cat))
    .slice(0, count)
}
