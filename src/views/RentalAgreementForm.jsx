...
import {
  FileText, User, Home, Users, MapPin, Phone, Check, AlertCircle, Wallet, ChevronRight, ChevronLeft, ArrowLeft, Calendar, ClipboardList
} from 'lucide-react'
...
const STEP_META = [
  { title: 'Agreement Type',     sub: 'Select the type of rental agreement',          Icon: FileText,      gradient: 'linear-gradient(135deg,#1A3D6E,#2E68B1)' },
  ...
  { title: 'Property Info',      sub: 'Details about the property and tenancy',       Icon: MapPin,        gradient: 'linear-gradient(135deg,#1A3D6E,#2E68B1)' },
...
const AGREEMENT_TYPES = [
  { value: 'Residential Rental Agreement', label: 'Residential Rental Agreement', desc: 'For apartments, flats, houses, and PGs', color: '#2E68B1', bg: '#EBF2FB', badge: 'Most Popular' },
  { value: 'Commercial Rental Agreement',  label: 'Commercial Rental Agreement',  desc: 'For offices, shops, and commercial spaces', color: '#059669', bg: '#ECFDF5', badge: null },
]
...
const STEPS = ['Type', 'Role', 'Owner', 'Tenant', 'Property', 'Details', 'Contact']
...
function Field({ label, children, optional = false }) {
  return (
    <div className="pf5-field">
      <label className="pf5-lbl">{label}{!optional && <span className="pf5-ast"> *</span>}</label>
      {children}
    </div>
  )
}
...
export default function RentalAgreementForm() {
  ...
  const validateStep = (s) => {
    if (s === 1 && !agreementType) return { msg: 'Please select an agreement type.', step: 1 }
    if (s === 2) {
    ...
    if (s === 3) {
      ...
      if (!ownerDistrict) return { msg: "Please select the owner's district.",       step: 3 }
    ...
    if (s === 4) {
      ...
      if (!tenantDistrict) return { msg: "Please select the tenant's district.",       step: 4 }
    ...
    if (s === 5) {
      ...
      if (!advanceAmount || isNaN(advanceAmount) || Number(advanceAmount) < 0)
        return { msg: 'Please enter a valid advance amount.', step: 5 }
    ...
    if (s === 6) {
      ...
      if (!paintingCharges) return { msg: 'Please select who bears the painting charges.', step: 6 }
    ...
    if (s === 7) {
      ...
      if (!shippingDistrict) return { msg: 'Please select the shipping district.', step: 7 }
  ...
  const next = () => {
    ...
    // Other steps: validate only the current step before advancing
  ...
  const submitPayment = async () => {
    ...
    const r = await paytmCheckout({
      orderId, amount: 50, service: "Rental Agreement", mobile, name,
    })
        ...
        <div className="pf5-summary-card">
          ...
          <div className="pf5-summary-rows">
            {[
              ...
              { label: 'Services',         value: agreementType  },
  ...
  return (
    <div className="pf5-page">
      <div className="pf5-breadcrumb">
        <Link href="/">Home</Link><span> / </span><Link href="/rental-agreement">Rental Agreement</Link><span> / </span><span>Form</span>
      </div>
      <div className="pf5-card">

        {/* Step dots — same as PanCardForm */}
        <div className="pf5-head-top">
          <div style={{ width: 32 }} />
        ...
        <div className="pf5-content">
          ...
          {step === 1 && (
            <div className="pf5-plan-list">
              {AGREEMENT_TYPES.map(({ value, label, desc, color, bg, badge }) => (
                <label key={value} className={`pf5-plan${agreementType === value ? ' sel' : ''}`}
                  style={agreementType === value ? { borderColor: color, background: bg } : {}}>
                  <input type="radio" name="agreementType" checked={agreementType === value}
                    onChange={() => setAgreementType(value)} style={{ display: 'none' }} />
                  <div className={`pf5-plan-radio${agreementType === value ? ' sel' : ''}`}
                    style={agreementType === value ? { background: color, borderColor: color } : {}}>
                    {agreementType === value && <Check size={10} strokeWidth={3} color="#fff" />}
                  </div>
            ...
            <>
              ...
              <Field label="State">
                <ValidInp valid={ownerState !== ''}>
                  <select className="pf5-inp" value={ownerState} onChange={e => { setOwnerState(e.target.value); setOwnerDistrict(''); setOwnerPin('') }}>
              ...
              <Field label="District">
                <ValidInp valid={ownerDistrict !== ''}>
                  <select className="pf5-inp" value={ownerDistrict} disabled={!ownerState}
                    onChange={e => { setOwnerDistrict(e.target.value); setError('') }}
                    style={!ownerState ? { opacity: .5, cursor: 'not-allowed' } : {}}>
                    <option value="">{ownerState ? 'Select District' : 'Select state first'}</option>
                    {getDistricts(ownerState).map(d => <option key={d}>{d}</option>)}
                  </select>
                </ValidInp>
              </Field>
            ...
            <>
              ...
              <Field label="State">
                <ValidInp valid={tenantState !== ''}>
                  <select className="pf5-inp" value={tenantState} onChange={e => { setTenantState(e.target.value); setTenantDistrict(''); setTenantPin('') }}>
                    <option value="">Select State</option>
                    {STATES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </ValidInp>
              ...
              <Field label="District">
                <ValidInp valid={tenantDistrict !== ''}>
                  <select className="pf5-inp" value={tenantDistrict} disabled={!tenantState}
                    onChange={e => { setTenantDistrict(e.target.value); setError('') }}
                    style={!tenantState ? { opacity: .5, cursor: 'not-allowed' } : {}}>
                    <option value="">{tenantState ? 'Select District' : 'Select state first'}</option>
                    {getDistricts(tenantState).map(d => <option key={d}>{d}</option>)}
                  </select>
                </ValidInp>
              </Field>
              <Field label="Pin Code">
              ...
              <Field label="Security Deposit">
                <ValidInp valid={securityDeposit !== '' && !isNaN(securityDeposit) && Number(securityDeposit) > 0}>
                  <input className="pf5-inp" type="text" placeholder="Enter security deposit amount"
                    value={securityDeposit} onChange={e => { setSecurityDeposit(e.target.value.replace(/[^0-9.]/g,'')); setError('') }} />
                </ValidInp>
              </Field>
              <Field label="Advance Amount">
                <ValidInp valid={advanceAmount !== '' && !isNaN(advanceAmount) && Number(advanceAmount) >= 0}>
                  <input className="pf5-inp" type="number" placeholder="Enter advance amount"
                    value={advanceAmount} onChange={e => { setAdvanceAmount(e.target.value); setError('') }} />
                </ValidInp>
              </Field>
              <Field label="Monthly Rent">
                <ValidInp valid={monthlyRent !== '' && !isNaN(monthlyRent) && Number(monthlyRent) > 0}>
                  <input className="pf5-inp" type="text" placeholder="Enter monthly rent amount"
                    value={monthlyRent} onChange={e => { setMonthlyRent(e.target.value.replace(/[^0-9.]/g,'')); setError('') }} />
                </ValidInp>
              </Field>
            </>
          )}
            ...
            <>
              ...
              <Field label="Painting Charges">
                <ValidInp valid={paintingCharges !== ''}>
                  <select className="pf5-inp" value={paintingCharges} onChange={e => setPaintingCharges(e.target.value)}>
                    <option value="">Choose</option>
            ...
            <>
              ...
              <Field label="State">
                <ValidInp valid={shippingState !== ''}>
                  <select className="pf5-inp" value={shippingState} onChange={e => { setShippingState(e.target.value); setShippingDistrict(''); setShippingPin('') }}>
                    <option value="">Select State</option>
                    {STATES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </ValidInp>
              ...
              <Field label="District">
                <ValidInp valid={shippingDistrict !== ''}>
                  <select className="pf5-inp" value={shippingDistrict} disabled={!shippingState}
                    onChange={e => { setShippingDistrict(e.target.value); setError('') }}
                    style={!shippingState ? { opacity: .5, cursor: 'not-allowed' } : {}}>
                    <option value="">{shippingState ? 'Select District' : 'Select state first'}</option>
                    {getDistricts(shippingState).map(d => <option key={d}>{d}</option>)}
                  </select>
                </ValidInp>
              </Field>
              <Field label="Pin Code">
