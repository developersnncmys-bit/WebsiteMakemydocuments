...
const ROLE_COLOURS = [
  { bg: 'var(--teal-bg)',  bd: 'var(--teal-bd)',  ic: 'var(--teal)'  },
  { bg: 'var(--amber-bg)', bd: 'var(--amber-bd)', ic: 'var(--amber)' },
  { bg: 'var(--green-bg)', bd: 'var(--green-bd)', ic: 'var(--green)' },
]

// Map a backend career doc to the shape the Careers page already renders.
function mapApiRole(c, i) {
  const colour = ROLE_COLOURS[i % ROLE_COLOURS.length]
  return {
    title:    c.title || '',
    dept:     c.department || 'General',
    type:     c.type || 'Full-time',
    location: c.location || 'Bangalore, KA',
    exp:      c.experience || '',
    desc:     c.description || '',
...
export default function Careers() {
  ...
  // Live roles from the backend (admin-managed). Falls back to BUNDLED_ROLES
  // only when the backend is unreachable (network error) — a successful
  // response with an empty array means "no open positions right now" and we
  // render the WhatsApp CTA below instead of stale bundled roles.
  const [OPEN_ROLES, setOpenRoles] = useState(BUNDLED_ROLES)
  const [rolesLoaded, setRolesLoaded] = useState(false)
  useEffect(() => {
    let cancelled = false
    fetch(`${API_BASE}/api/careers?status=open`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((json) => {
        if (cancelled) return
        const data = Array.isArray(json?.data) ? json.data : []
        setOpenRoles(data.map(mapApiRole))
      })
    ...
    <main style={{ paddingTop: 66 }}>
      ...
      <section className="car-roles" id="open-roles">
        <div className="mx">
          ...
          {/* No-roles fallback — admin hasn't added any open positions yet.
              We surface a direct WhatsApp link so visitors can still reach us. */}
          {rolesLoaded && OPEN_ROLES.length === 0 && (
            <div
              style={{
            ...
            >
              <Briefcase size={36} strokeWidth={1.6} color="var(--teal)" style={{ marginBottom: 12 }} />
              <h3 style={{ marginBottom: 8 }}>No open positions right now</h3>
              <p style={{ color: 'var(--ink3)', fontSize: 15, marginBottom: 20 }}>
                We&apos;re always interested in great people. Ping us on WhatsApp and
                tell us how you&apos;d like to contribute.
              </p>
              <a
                href="https://wa.me/919980097315?text=Hi! I'd like to explore career opportunities at MakeMyDocuments."
                target="_blank"
                rel="noreferrer"
                className="btn-teal"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}
              >
                Reach us on WhatsApp <ArrowRight size={15} strokeWidth={2.5} />
              </a>
            </div>
          )}
          ...
          <div className="car-roles-grid">
            {OPEN_ROLES.map((role, i) => (
              <div key={role.title} className="car-role-card" style={{ transitionDelay: `${i * 60}ms` }}>
          ...
          <div className="car-roles-note rv">
            ...
            <a
              href="mailto:support@makemydocuments.com?subject=Resume%20for%20MakeMyDocuments&body=Hi%20Team%2C%0A%0AI%27d%20like%20to%20be%20considered%20for%20future%20openings.%20My%20resume%20is%20attached.%0A%0AThanks!"
