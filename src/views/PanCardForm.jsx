...
import {
  ...
  FileText, AlertCircle, Edit3, AlertTriangle, Wallet,
...
export default function PanCardForm() {
  ...
  const submitPayment = async () => {
    ...
    const r = await paytmCheckout({
      orderId, amount: PRICE, service: 'PAN Card', mobile, name,
    })
    if (!r.ok) { setError(r.error || 'Could not start payment.'); setPayBusy(false) }
    // On success the browser navigates to Paytm — nothing else to do here.
  }
        ...
        <div className="pf5-card">
          ...
          <div className="pf5-footer">
            ...
            <button className="pf5-cta-btn pf5-pay" onClick={submitPayment} disabled={payBusy}>
              <Check size={15} /> {payBusy ? 'Redirecting…' : `Pay ₹${PRICE} with Paytm`}
            </button>
          </div>
            ...
            <>
              <Field label="Address">
                <ValidInp valid={house.trim().length > 0}>
                  <input className="pf5-inp" type="text" placeholder="Enter your address" value={house} onChange={e => setHouse(e.target.value)} />
                </ValidInp>
              </Field>
              <Field label="State">
