...
import { fetchBlogBySlug, fetchBlogPosts, getRelatedPosts } from '../data/blogPosts'
import { getStates, getDistricts, validatePincode } from '../data/locationData'
import { submitLead } from '../lib/submitLead'

// Canonical 11 backend services — kept in sync with Solution.jsx,
// ContactUs.jsx and the admin's lib/constants.ts.
const SERVICES = [
  'Passport',
  'PAN Card',
  'Tourist Visa',
...
function ContactForm() {
  ...
  const onSubmit = async e => {
    ...
    await submitLead({
      ...
      source: 'Blog Contact',
      formData: { enquiryService: form.service, state: form.state, district: form.district, pinCode: form.pincode, source: 'Blog Detail Page' },
    })
    setLoading(false)
    setSent(true)
  }
...
export default function BlogDetail() {
  ...
  useEffect(() => {
    let active = true
    ;(async () => {
      // Resolve the slug from the real URL, NOT useParams(). On Netlify, blogs
      // added after the last build are served the pre-built "__blog_fallback"
      // shell via a rewrite, so useParams() would return "__blog_fallback"
      // instead of the slug actually in the address bar. Reading the pathname
      // makes every blog (old or newly added) load correctly.
      const fromPath = typeof window !== 'undefined'
        ? decodeURIComponent(window.location.pathname.split('/').filter(Boolean).pop() || '')
        : ''
      const slug = (fromPath && fromPath !== '__blog_fallback') ? fromPath : params.slug
      ...
      if (active) setRelated(getRelatedPosts(all, found.id, found.cat, 6))
    })()
    return () => { active = false }
  }, [params.slug, router])
  ...
  return (
    // Top padding clears the fixed Nav (66px tall) so the hero/breadcrumb isn't
    // hidden behind it. Inline so it always wins over the bd-page CSS class.
    <div className="bd-page" style={{ paddingTop: 80 }}>

      {/* Reading progress */}
      <div className="bd-progress" style={{ width: `${progress}%` }} />

      {/* ── Hero ── */}
      <div className="bd-mag-hero" style={{ paddingTop: 24 }}>
      ...
      {/* ── Body ──
          Two-column layout: article on the LEFT, sticky contact form on the
          RIGHT. The form stays in view while the user scrolls the article.
          On screens narrower than 900px the form drops below the article. */}
      <style>{`
        .bd-body-grid--sticky {
          display: grid;
        ...
        @media (max-width: 900px) {
          .bd-body-grid--sticky { grid-template-columns: 1fr; }
          .bd-body-grid--sticky .bd-sidebar-sticky { position: static !important; }
        }
      `}</style>
      <div
        className="bd-body-wrap"
        style={{ maxWidth: 1320, width: '100%', padding: '40px clamp(16px, 4vw, 40px) 72px', margin: '0 auto' }}
      >
        <div className="bd-body-grid--sticky">
          ...
          {/* RIGHT — sticky contact CTA */}
          <aside
            className="bd-sidebar bd-sidebar-sticky"
            style={{ position: 'sticky', top: 86, alignSelf: 'start' }}
          >
            ...
            {/* Trust badges */}
            <div className="bd-trust-strip" style={{ marginTop: 16 }}>
