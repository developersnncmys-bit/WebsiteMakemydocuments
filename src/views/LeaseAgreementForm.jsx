...
import {
  FileText, Home, Users, MapPin, Phone, Check, AlertCircle, Wallet, ChevronRight, ChevronLeft, ArrowLeft, Calendar, ClipboardList
} from 'lucide-react'
...
export default function LeaseAgreementForm() {
  ...
  const submitPayment = async () => {
    ...
    const r = await paytmCheckout({
      orderId, amount: 50, service: "Lease Agreement", mobile, name,
    })
  ...
  return (
    <div className="pf5-page">
      <div className="pf5-breadcrumb">
        <Link href="/">Home</Link><span> / </span><Link href="/lease-agreement">Lease Agreement</Link><span> / </span><span>Form</span>
      </div>
      <div className="pf5-card">
              ...
              <Field label="State">
                <ValidInp valid={ownerState !== ''}>
                  <select className="pf5-inp" value={ownerState}
                    onChange={e => { setOwnerState(e.target.value); setOwnerDistrict(''); setOwnerPin('') }}>
              ...
              <Field label="State">
                <ValidInp valid={tenantState !== ''}>
                  <select className="pf5-inp" value={tenantState}
                    onChange={e => { setTenantState(e.target.value); setTenantDistrict(''); setTenantPin('') }}>
                    <option value="">Select State</option>
                    {STATES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </ValidInp>
              ...
              <Field label="State">
                <ValidInp valid={shippingState !== ''}>
                  <select className="pf5-inp" value={shippingState}
                    onChange={e => { setShippingState(e.target.value); setShippingDistrict(''); setShippingPin('') }}>
                    <option value="">Select State</option>
                    {STATES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </ValidInp>
